...
ecs_in = np.load(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{ens_constraint_set}/prior_runs/ecs.npy", mmap_mode='r'
)
tcr_in = np.load(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{ens_constraint_set}/prior_runs/tcr.npy", mmap_mode='r'
)
#af_in = np.load(
...
samples["TCR"] = scipy.stats.norm.rvs(
    loc=1.84, scale=0.6 / NINETY_TO_ONESIGMA, size=10**5, random_state=18196
)
...
model_run = ens_run-1
ohca_later =Dataset("../../../../../data/cmip-thorne/ssp126_ohca.nc", 'r').variables['__xarray_dataarray_variable__']
ohca_l = ohca_later[:].__array__()
ohca_ly = average_every_n(ohca_l[model_run,:], 12) 
ohca_earlier = Dataset("../../../../../data/cmip-thorne/MPI_ESM_historical_ohca.nc", 'r').variables['__xarray_dataarray_variable__']
...
ohca_sc_factor = max(1,(float(cutoff_env)-2014+40)/40)
samples["OHC"] = scipy.stats.norm.rvs(
    loc=(ohca_meas[cutoff_int]-ohca_meas[cutoff_int-50]),
    scale=np.sqrt(ohc_sd[i1ohc]**2 + ohc_sd[i2ohc]**2)*ohca_sc_factor, size=10**5, random_state=43178)
...
samples["temperature l20yrs"] = scipy.stats.norm.rvs(
        ...
        scale=ohca_sc_factor*np.sqrt(preind_var_unc+preind_var_int+recent_var_unc+recent_var_int), 
...
df_co2_model = pd.read_csv("../../../../../data/cmip-thorne/ERF_ESM1-2-LR_ssp126.csv")
co2_sc_orig = 4*df_co2['unc'].values[i1ohc]
#widen it from 0.5 to 5 gradually from 2020-2060
co2_sc_factor = max(1,(float(cutoff_env)-2014+4)/4)
samples["CO2 concentration"] = scipy.stats.norm.rvs(
    loc= df_co2_model['CO2'].values[cutoff_int], scale=co2_sc_orig*co2_sc_factor,
