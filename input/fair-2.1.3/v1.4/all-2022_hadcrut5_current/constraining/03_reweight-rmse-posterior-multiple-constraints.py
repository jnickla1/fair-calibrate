...
constraint_set = os.getenv("CONSTRAINT_SET")+"cut"+os.getenv("CUTOFF_YEAR")
...
temp_in = np.load(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{orig_const}/prior_runs/"
    "temperature_1850-2101.npy"
)
ohc_in_all = np.load(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{orig_const}/prior_runs/"
    "ocean_heat_content_1850-2101.npy"
)
...
co2_in_all = np.load(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{orig_const}/prior_runs/"
    "concentration_co2_1850-2101.npy"
)
#need to change this co2 to save to the most recent year
ecs_in = np.load(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{orig_const}/prior_runs/ecs.npy"
)
tcr_in = np.load(
    f"../../../../../output/fair-{fair_v}/v{cal_v}/{orig_const}/prior_runs/tcr.npy"
)
#af_in = np.load(
...
recent_var_unc =((-np.mean( df_gmst["Lower confidence limit (2.5%)"].values[(cutoff_int-18):(cutoff_int+2)]) +
          np.mean( df_gmst["Upper confidence limit (97.5%)"].values[(cutoff_int-18):(cutoff_int+2)]))/4)**2 
...
samples["CO2 concentration"] = scipy.stats.norm.rvs(
    loc=df_co2['mean'].values[cutoff_int], scale=4*df_co2['unc'].values[cutoff_int],
